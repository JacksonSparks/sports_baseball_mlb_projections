                    ...
                    if tbody:
                        ...
                        for tr in tr_elements:
                            ...
                            if len(td_elements) >= 13:
                                span = td_elements[0].find('span', class_='fw-bold clr-gray-01 ttu')
                                if span:
                                    if span.text.strip() == 'Totals':
...
def main():
    ...
    for index, row in lineups_data.iterrows():
        ...
        for i, batter in enumerate(away_batters, 1):
            ...
            if link:
                batter_stats = fetch_batter_vs_pitcher_table(link)
                if not batter_stats:
                    # If no stats found, append row with zeros
                    batter_stats = [['0'] * 12]
                for stats in batter_stats:
                    batter_location = 'Away'
                    data_rows.append([full_name, home_team_name, batter_location, batter_spot] + stats)
        ...
        for i, batter in enumerate(home_batters, 1):
            ...
            if link:
                ...
                for stats in batter_stats:
                    ...
                    data_rows.append([full_name, away_team_name, batter_location, batter_spot] + stats)

    # Writing collected data to a CSV file
    with open('mlb_battervteam_hth_data_a.csv', 'w', newline='', encoding='utf-8') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(['Batter', 'Opposing Team', 'Batter Location', 'Batter Spot', 'AB', 'H', '2B', '3B', 'HR', 'RBI', 'BB', 'K', 'AVG', 'OBP', 'SLG', 'OPS'])
